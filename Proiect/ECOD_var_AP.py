...
def generate_table_3_ap(datasets):
    results = []

    print(f"\nReplicare TABEL 3 (Average Precision - AP)")
    print(f"{'Dataset':<20} | {'ECOD-L':<8} | {'ECOD-R':<8} | {'ECOD-B':<8} | {'ECOD':<8}")
    print("-" * 65)
    ...
    for ds_name in datasets:
        try:
            ...
            ap_l = average_precision_score(y_test, score_L)
            ap_r = average_precision_score(y_test, score_R)
            ap_b = average_precision_score(y_test, score_B)
            ap_auto = average_precision_score(y_test, score_Auto)
            ...
            print(f"{ds_name.replace('.npz', ''):<20} | {ap_l:.4f}   | {ap_r:.4f}   | {ap_b:.4f}   | {ap_auto:.4f}")

        except FileNotFoundError:
            print(f"Eroare: Fișierul {ds_name} lipsește.")
...
if __name__ == "__main__":
    df_ap = generate_table_3_ap(files_list)
    
    if df_ap is not None:
        df_ap.to_csv('tabel_3_average_precision.csv', index=False)
